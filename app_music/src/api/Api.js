...
function search(
  ...
  if (type !== "All") {
    return instance
      .get(
        `recording?query=${type}:"${wordsearch}"&offset=${offset}&limit=${limit}`,
        {
    ...
    return instance
      .get(
        `recording?query=artist:"${wordsearch}"%20OR%20release:"${wordsearch}"%20OR%20recording:"${wordsearch}"&offset=${offset}&limit=${limit}`,
        {
          cancelToken: source.token,
        }
      )
...
function searchSeeMore(id) {
  return axios
    .get(
      `http://musicbrainz.org/ws/2/recording/${id}?inc=genres+ratings+artists+releases`
    )
...
async function searchCover(result, offset, cover) {
  ...
  if (offset + 1 <= ids.length) {
    return await axios
      .get(`https://coverartarchive.org/release/${ids[offset]}`)
      .then(function (response) {
        if (response.status === 200 && response.data) {
